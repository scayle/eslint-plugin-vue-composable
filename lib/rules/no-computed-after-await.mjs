/**
 * Based on the no-watch-after-await rule in eslint-plugin-vue, adapted for composables and computed
 * @see https://github.com/vuejs/eslint-plugin-vue/blob/master/lib/rules/no-watch-after-await.js
 */

import { isComposableFunction } from '../utils.mjs'
...
export default {
  meta: {
    ...
    docs: {
      description:
        'disallow asynchronously registered `computed` in composables',
      recommended: true,
    },
    fixable: null,
    schema: [],
  ...
  create(context) {
    ...
    return {
      ...
      CallExpression(node) {
        ...
        if (
          'name' in node.callee && node.callee.name === 'computed'
        ) {
